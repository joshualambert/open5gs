...
| static_orphan | 5.3: static UE IPv6 `2001:db8:dead::1` (outside every subnet), IPv6-only PDN type, is rejected, not routed into the wrong pool: EPC Attach Reject with EMM cause #17 (network failure) followed by UE Context Release; 5GC PDU Session Establishment Reject with 5GSM cause #67 in a DownlinkNASTransport, the UE stays registered and de-registers cleanly. (With PDN type IPv4v6 the SMF's historical family fallback downgrades the session to IPv4-only instead of rejecting; that path is not asserted.) |
